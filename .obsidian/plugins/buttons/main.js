...
var nameModal = class extends import_obsidian2.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian2.Setting(contentEl).setName("Name").addText((text2) => {
      text2.setPlaceholder(this.defaultName);
      text2.onChange((value) => {
        this.result = value;
      });
    });
    new import_obsidian2.Setting(contentEl).addButton((btn) => btn.setButtonText("Create Note").setCta().onClick(() => {
      this.close();
...
async function templater(app, template2, _target) {
  ...
  if (!activeFile) {
    new import_obsidian3.Notice("No active file found for templater processing.");
    return;
  }
  const config = {
  ...
  if (!exists) {
    new import_obsidian3.Notice("Templater is not installed. Please install it.");
    return;
  }
  try {
    ...
    new import_obsidian3.Notice("Error setting up templater functions. Check console for details.");
    return;
  }
}
async function processTemplate(app, file) {
  try {
    ...
    new import_obsidian3.Notice(`There was an error processing the template!`, 2e3);
  }
}
...
var removeButton = async (app, remove2, lineStart, lineEnd) => {
  ...
  if (lineStart === lineEnd) {
    ...
    handleValueArray(remove2, async (argArray) => {
      const buttons = store && store.filter((item) => {
        let exists;
...
var prependContent = async (app, insert, lineStart, isTemplater) => {
  ...
  if (activeView) {
    ...
    new import_obsidian4.Notice("There was an issue prepending content, please try again", 2e3);
  }
};
...
var addContentAtLine = async (app, insert, type, isTemplater) => {
  ...
  if (lineNumber[0]) {
    ...
    new import_obsidian4.Notice("There was an issue adding content, please try again", 2e3);
  }
};
var createNote = async (app, type, folder, prompt, filePath, isTemplater) => {
  ...
  if (path) {
    ...
    try {
      ...
      new import_obsidian4.Notice("There was an error! Maybe the file already exists?", 2e3);
    }
  } else {
    new import_obsidian4.Notice(`couldn't parse the path!`, 2e3);
  }
};
  ...
  if (variables) {
    const output = variables.map(async (value) => {
      ...
      if (activeView) {
        ...
        new import_obsidian5.Notice(`couldn't read file`, 2e3);
      }
    });
    ...
    resolved.forEach((term) => {
      if (term.numbers) {
        ...
        new import_obsidian5.Notice("Check the line number in your calculate button", 3e3);
        equation = void 0;
      }
    });
  }
...
var template = async (app, args, position) => {
  ...
  if (templatesEnabled || templaterPluginEnabled) {
    ...
    if (file) {
      ...
      new import_obsidian5.Notice(`Couldn't find the specified template, please check and try again`, 2e3);
    }
  } else {
    new import_obsidian5.Notice("You need to have the Templates or Templater plugin enabled and Template folder defined", 2e3);
  }
};
var link = ({ action }) => {
...
var command = (app, args, buttonStart) => {
  ...
  if (!command2) {
    new import_obsidian5.Notice(`Command "${action}" not found. Available commands can be found in the Command Palette.`);
  ...
  try {
    if (args.type.includes("prepend")) {
      app.workspace.getActiveViewOfType(import_obsidian5.MarkdownView).editor.setCursor(buttonStart.lineStart, 0);
      app.commands.executeCommandById(command2.id);
    }
    if (args.type.includes("append")) {
      app.workspace.getActiveViewOfType(import_obsidian5.MarkdownView).editor.setCursor(buttonStart.lineEnd + 2, 0);
      app.commands.executeCommandById(command2.id);
    }
    ...
    new import_obsidian5.Notice(`Failed to execute command "${action}": ${error.message}`);
  ...
  handleValueArray(swap2, async (argArray) => {
    ...
    if (args) {
      if (args.templater) {
        ...
        if (inline) {
          new import_obsidian5.Notice("templater args don't work with inline buttons yet", 2e3);
        }
      }
      if (args.replace) {
...
var clickHandler = async (app, args, inline, id) => {
  ...
  if (!activeFile) {
    new import_obsidian6.Notice("No active file found. Buttons can only be used with files.");
    return;
  }
  ...
  if (inline && args.templater && args.action && args.action.includes("<%")) {
    try {
      ...
      new import_obsidian6.Notice("Error processing templater in inline button. Check console for details.", 2e3);
    }
  }
  if (args.replace) {
    replace(app, args);
  ...
  if (args.swap) {
    if (!inline) {
      new import_obsidian6.Notice("swap args only work in inline buttons for now", 2e3);
    } else {
...
var TemplateSuggest = class extends TextInputSuggest {
  ...
  getSuggestions(inputStr) {
    ...
    abstractFiles.forEach((file) => {
      ...
      if (file instanceof import_obsidian7.TFile && file.extension === "md" && exists && file.path.toLowerCase().contains(lowerCaseInputStr)) {
        files.push(file);
      }
    });
    return files;
...
// src/modal.ts
var ButtonModal = class extends import_obsidian8.Modal {
  constructor(app) {
    super(app);
  ...
  onOpen() {
    ...
    contentEl.createEl("form", {}, (formEl) => {
      new import_obsidian8.Setting(formEl).setName("Button Name").setDesc("What would you like to call this button?").addText((textEl) => {
      ...
      new import_obsidian8.Setting(typeDesc).addDropdown((drop) => {
        ...
        drop.onChange((value) => {
          ...
          if (value === "command") {
            action.empty();
            new import_obsidian8.Setting(action).setName("Command").setDesc("Enter a command to run").addDropdown((drop2) => {
          ...
          if (value.includes("template")) {
            action.empty();
            new import_obsidian8.Setting(action).setName("Template").setDesc("Select a template note and what should happen").addDropdown((drop2) => {
              ...
              drop2.onChange((value2) => {
                ...
                if (value2 == "line template") {
                  new import_obsidian8.Setting(action).setName("Line Number").setDesc("At which line should the template be inserted?").addText((textEl) => {
                ...
                if (value2 == "note template") {
                  new import_obsidian8.Setting(action).setName("Prompt").setDesc("Should you be prompted to enter a name for the file on creation?").addToggle((toggleEl) => {
                    this.outputObject.prompt = false;
                    toggleEl.onChange((bool) => this.outputObject.prompt = bool);
                  });
                  new import_obsidian8.Setting(action).setName("Note Name").setDesc("What should the new note be named? Note: if prompt is on, this will be the default name").addText((textEl) => {
                    textEl.setPlaceholder("My New Note");
                    new import_obsidian8.Setting(action).setName("Default Folder").setDesc("Enter a folder path to place the note in. Defaults to root").addText((textEl2) => {
                      this.outputObject.folder = "";
                      textEl2.onChange((textVal) => {
                        this.outputObject.folder = textVal;
                      });
                    });
                    new import_obsidian8.Setting(action).setName("Split").setDesc("Should the new note open in a split pane?").addToggle((toggleEl) => {
          ...
          if (value.includes("text")) {
            action.empty();
            new import_obsidian8.Setting(action).setName("Text").setDesc("What text and where should it go?").addDropdown((drop2) => {
              drop2.addOption("pre", "Do this...");
              ...
              drop2.onChange((value2) => {
                ...
                if (value2 == "note text") {
                  new import_obsidian8.Setting(action).setName("Note Name").setDesc("What should the new note be named?").addText((textEl) => {
                    textEl.setPlaceholder("My New Note");
                    new import_obsidian8.Setting(action).setName("Split").setDesc("Should the new note open in a split pane?").addToggle((toggleEl) => {
          ...
          if (value === "calculate") {
            action.empty();
            new import_obsidian8.Setting(action).setName("Calculate").setDesc("Enter a calculation, you can reference a line number with $LineNumber").addText((textEl) => {
          ...
          if (value === "swap") {
            ...
            action.empty();
            new import_obsidian8.Setting(action).setName("Swap").setDesc("choose buttons to be included in the Inline Swap Button").addText((textEl) => {
              textEl.inputEl.replaceWith(this.swapSuggestEl);
            });
          }
          if (value === "copy") {
            action.empty();
            new import_obsidian8.Setting(action).setName("Text").setDesc("Text to copy for clipboard").addText((textEl) => {
      ...
      new import_obsidian8.Setting(formEl).setName("Button Block ID").setDesc("Provide a custom button-block-id").addText((textEl) => {
        textEl.setPlaceholder("buttonId");
        textEl.onChange((value) => {
          this.outputObject.blockId = value;
        });
      });
      new import_obsidian8.Setting(formEl).setName("Remove").setDesc("Would you like to remove this button (or other buttons) after clicking?").addToggle((toggleEl) => {
        toggleEl.onChange((value) => {
          if (value) {
            new import_obsidian8.Setting(remove2).setName("Select Remove").setDesc("Use true to remove this button, or supply an [array] of button block-ids").addText((textEl) => {
      ...
      new import_obsidian8.Setting(formEl).setName("Replace").setDesc("Would you like to replace lines in the note after clicking?").addToggle((toggleEl) => {
        toggleEl.onChange((value) => {
          if (value) {
            new import_obsidian8.Setting(replace2).setName("Select Lines").setDesc("Supply an array of [startingLine, endingLine] to be replaced").addText((textEl) => {
              textEl.setValue("[]");
      ...
      new import_obsidian8.Setting(formEl).setName("Inherit").setDesc("Would you like to inherit args by adding an existing button block-id?").addToggle((toggleEl) => {
        toggleEl.onChange((value) => {
          if (value) {
            new import_obsidian8.Setting(id).setName("id").setDesc("inherit from other Buttons by adding their button block-id").addText((textEl) => {
      ...
      new import_obsidian8.Setting(formEl).setName("Templater").setDesc("Do you want to convert a templater command inside your Button on each click?").addToggle((toggleEl) => {
        toggleEl.setTooltip("Do not use for inline Button");
        toggleEl.onChange((value) => {
          this.outputObject.templater = value;
        });
      });
      new import_obsidian8.Setting(formEl).setName("Custom Class").setDesc("Add a custom class for button styling").addText((textEl) => {
        textEl.onChange((value) => {
      ...
      new import_obsidian8.Setting(formEl).setName("Color").setDesc("What color would you like your button to be?").addDropdown((drop) => {
        ...
        drop.onChange((value) => {
          ...
          if (value === "custom") {
            ...
            new import_obsidian8.Setting(customBackgroundColor).setName("Background: ").addText((el) => {
            ...
            new import_obsidian8.Setting(customTextColor).setName("Text Color: ").addText((el) => {
          ...
          const buttonClass = this.buttonPreviewEl.getAttribute("class").replace(" blue", "").replace(" red", "").replace(" green", "").replace(" yellow", "").replace(" purple", "");
          if (value !== "default") {
...
var InlineButtonModal = class extends import_obsidian8.Modal {
  constructor(app) {
    super(app);
...
var ButtonWidget = class extends import_view.WidgetType {
  ...
  async handleButtonClick(args) {
    ...
    if (!activeFile) {
      new import_obsidian9.Notice("No active file found. Buttons can only be used with files.");
      return;
    }
    if (args.templater && args.action && args.action.includes("<%")) {
      try {
        ...
        console.error("Error processing templater in inline button:", error);
        new import_obsidian9.Notice("Error processing templater in inline button. Check console for details.", 2e3);
      }
    }
...
// src/index.ts
var ButtonsPlugin = class extends import_obsidian10.Plugin {
  constructor() {
    super(...arguments);
    ...
    __publicField(this, "storeEvents", new import_obsidian10.Events());
...
var InlineButton = class extends import_obsidian10.MarkdownRenderChild {

...
LOCALE="pt_BR.UTF-8"
KEYMAP="br-abnt2"    # Layout do teclado do console (para uso no console, menos relevante para GNOME em WSL)
HOSTNAME="fedora-gnome-dev-wsl"
...
# --- Início do Script ---
print_header "Iniciando Configuração Completa do Fedora WSL: GNOME, Node.js, Fish, Fisher, Obsidian, VS Code e Cursor"

# 1. Verificação da Distro
if ! grep -q "ID=fedora" /etc/os-release; then
    print_error "Este script é para Fedora Linux. Sua distro não é Fedora."
    exit 1
fi

# 2. Verificação de Permissões Sudo (assumindo que o usuário já tenha sido criado e configurado na primeira execução do Fedora Remix)
if ! groups "$USERNAME" | grep -q "wheel" && ! groups "$USERNAME" | grep -q "sudo"; then
    print_warning "Usuário '$USERNAME' não está no grupo 'wheel' ou 'sudo'. Este script requer permissões sudo."
    print_error "Por favor, adicione seu usuário ao grupo 'wheel' (sudo usermod -aG wheel $USERNAME) e reinicie o WSL antes de executar este script."
    exit 1
fi
...
sudo dnf upgrade --refresh -y || { print_error "Falha ao atualizar o sistema."; exit 1; }
print_info "Sistema Fedora WSL atualizado com sucesso!"
...
if confirm_action "Configurar Localização para $LOCALE"; then
    ...
    sudo dnf install -y glibc-langpack-"$(echo "$LOCALE" | cut -d'.' -f1 | cut -d'_' -f1)" || print_warning "Falha ao instalar pacote de idioma." # Adaptação para o formato do pacote glibc-langpack
    sudo localectl set-locale LANG="$LOCALE" || print_warning "Falha ao definir locale."
...
# --- 5. Instalar Interface Gráfica GNOME ---
print_header "5. Instalação da Interface Gráfica GNOME"
if confirm_action "GNOME Desktop Environment e componentes essenciais (grupo 'Workstation')"; then
    print_info "Instalando grupo 'Workstation' (GNOME) e Display Manager GDM..."
    # Adicionando redundância: Tenta instalar o grupo Workstation, se falhar, tenta instalar o GNOME core
    sudo dnf groupinstall -y "Workstation" || \
    (print_warning "Primeira tentativa de instalação do Workstation falhou. Tentando instalar 'gnome-shell' e 'gdm'..." && sudo dnf install -y gnome-shell gdm) || \
    { print_error "Falha crítica na instalação do GNOME Desktop. Verifique a conexão e os repositórios."; exit 1; }
    print_info "GNOME Desktop instalado."

    print_info "Habilitando GDM (Display Manager) para iniciar o ambiente gráfico..."
    sudo systemctl enable gdm --now || print_warning "Falha ao habilitar/iniciar GDM. Verifique se o systemd está ativo."
    print_info "GDM habilitado."
else
...
sudo dnf install -y pulseaudio-utils pavucontrol || print_warning "Falha ao instalar PulseAudio. O áudio pode não funcionar."
...
if confirm_action "Node.js via Node Version Manager (NVM)"; then
    print_info "Instalando dependências para NVM (curl, wget)..."
    sudo dnf install -y curl wget || print_warning "Falha ao instalar curl/wget."
...
if confirm_action "Shell Fish"; then
    ...
    sudo dnf install -y fish || print_error "Falha ao instalar Fish."
    ...
    echo "set -g fish_greeting 'Bem-vindo ao Fish Shell no Fedora Linux WSL!'" > "$HOME/.config/fish/config.fish"
...
if confirm_action "Visual Studio Code"; then
    print_info "Adicionando o repositório oficial do VS Code..."
    # Importa a chave GPG da Microsoft
    sudo rpm --import https://packages.microsoft.com/keys/microsoft.asc || print_warning "Falha ao importar chave GPG do VS Code."
    # Adiciona o repositório do VS Code
    sudo sh -c 'echo -e "[code]\nname=Visual Studio Code\nbaseurl=https://packages.microsoft.com/yumrepos/vscode\nenabled=1\ngpgcheck=1\ngpgkey=https://packages.microsoft.com/keys/microsoft.asc" > /etc/yum.repos.d/vscode.repo' || print_error "Falha ao adicionar repositório do VS Code."
    
    print_info "Instalando Visual Studio Code..."
    sudo dnf check-update || print_warning "Falha ao checar atualizações após adicionar repositório do VS Code."
    sudo dnf install -y code || print_error "Falha ao instalar Visual Studio Code."
...
if confirm_action "Cursor"; then
    print_info "Baixando o pacote RPM do Cursor..."
    # URL para a última versão do Cursor (pode precisar ser atualizada periodicamente)
    ...
    # A URL abaixo busca o pacote mais recente para x86_64 diretamente do site oficial, que é o método mais robusto.
    CURSOR_URL="https://releases.cursor.sh/linux/rpm/x86_64"
    # Este comando tenta encontrar a URL exata do RPM mais recente na página de releases ou um padrão conhecido
    LATEST_CURSOR_RPM=$(curl -s $CURSOR_URL | grep -oP 'cursor-\d+\.\d+\.\d+[^"]*\.rpm' | head -n 1)
    
    if [ -n "$LATEST_CURSOR_RPM" ]; then
        wget -O "$HOME/cursor.rpm" "${CURSOR_URL}/${LATEST_CURSOR_RPM}" || \
        (print_warning "Primeira tentativa de download do Cursor falhou. Tentando novamente..." && wget -O "$HOME/cursor.rpm" "${CURSOR_URL}/${LATEST_CURSOR_RPM}") || \
        { print_error "Falha ao baixar Cursor RPM. Verifique a URL ou sua conexão."; }

        if [ -f "$HOME/cursor.rpm" ]; then
            print_info "Instalando Cursor RPM..."
            sudo dnf install -y "$HOME/cursor.rpm" || print_error "Falha ao instalar Cursor via DNF."
            print_info "Limpando pacote RPM do Cursor..."
            rm "$HOME/cursor.rpm"
            print_info "Cursor instalado."
        else
            print_error "Pacote RPM do Cursor não foi baixado. Instalação ignorada."
        fi
    else
        print_error "Não foi possível encontrar a URL do pacote RPM mais recente do Cursor. Instalação ignorada."
    fi
else
...
sudo dnf clean all -y || print_warning "Falha ao limpar cache do DNF."
sudo dnf autoremove -y || print_warning "Nenhum pacote órfão para remover ou falha na remoção."
print_info "Limpeza concluída."

print_header "Processo de Configuração de Fedora WSL Concluído!"
print_warning "Para aplicar todas as mudanças (GUI, Fish como shell padrão, NVM, novos editores), é **fortemente recomendado reiniciar sua instância Fedora WSL**."
print_warning "Feche todos os terminais Fedora WSL e, no PowerShell do Windows, execute 'wsl --shutdown'."
print_warning "Em seguida, reabra sua instância Fedora ('wsl -d FedoraWSL')."
print_warning "Após reiniciar, você deverá ver a tela de login do GNOME na janela do seu X Server (VcXsrv)."
